/* ============================================================
   MAP UI MODULE — v2
   FULLY REPAIRED + SYNCHRONIZED HUD ARCHITECTURE
   ============================================================ */

console.log("map-ui.js loaded");

/* ------------------------------------------------------------
   GLOBAL MARKER REGISTRIES
------------------------------------------------------------ */
window.MARKERS = {};
window.POPUPS  = {};
window.MINOR_MARKERS = [];

/* ------------------------------------------------------------
   TRAVEL MODE ICONS (SAFE + FUTURE-PROOF)
------------------------------------------------------------ */
const MODE_ICONS = {
      ...
      detailsWeatherContent.innerHTML = `
        <div class="details-weather-main">
          ...
          <div class="details-weather-temp">
            ${tempF}°F <span style="color:#A3A3A3">(${tempC.toFixed(1)}°C)</span>
          </div>
        </div>
        <div class="details-weather-meta">
          <div class="details-weather-label">${info.label}</div>
          <div class="details-weather-wind">
            Wind: ${windM} mi/h <span style="color:#A3A3A3">(${windK} km/h)</span>
          </div>
        </div>
      `;
    })
...
function openDetailsSidebar(id) {
  ...
  detailsLocation.innerHTML =
    `${escapeHTML(w.location)} <span class="details-location-flag-inline" style="background-image:url('${w.meta.flag}')"></span>`;
...
detailsClose.onclick = closeDetailsSidebar;
...
document.addEventListener("keydown", e => {
  if (e.key === "Escape") closeDetailsSidebar();
});

document.addEventListener("click", e => {
  const btn = e.target.closest(".details-btn");
  if (btn) openDetailsSidebar(btn.dataset.details);
});
...
/* -------------------------------------------------------------------------
   SECTION 3 — MAIN HUD (Journey HUD)
-------------------------------------------------------------------------- */
...
function updateHUD() {
  ...
  updateDetailsHud(); // Keep both HUD systems synced
}
...
/* -------------------------------------------------------------------------
   SECTION 4 — POI SYSTEM (Disabled — future support)
-------------------------------------------------------------------------- */
