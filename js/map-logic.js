/* ==========================================================================
   MAP LOGIC MODULE — FINAL MODULAR VERSION (CINEMATIC ENABLED)
   ========================================================================== */
...
/* ==========================================================================
   DISTANCE CALCULATIONS — SAME AS ORIGINAL BUT STABLE
========================================================================== */
...
window.buildGreatCircle = function (fromId, toId, steps = 220) {
    ...
    if (!Number.isFinite(Δ) || Δ === 0) {
        return [
            normalizeCoord(lon1d, lat1),
            normalizeCoord(lon2d, lat2)
        ];
    }
...
/* ==========================================================================
   STATIC ROUTES (Handled by map-style.js)
========================================================================== */

window.addStaticRoutes = function () {
    console.log("addStaticRoutes(): handled by map-style.js");
};

/* ==========================================================================
   DRIVING ROUTE — MAPBOX DIRECTIONS
========================================================================== */
...
window.buildDrivingRoute = async function () {
    ...
    const src = map.getSource("drive-route");
    if (src) src.setData(json.routes[0].geometry);
...
/* ==========================================================================
   JOURNEY SOURCE LAYERS — CREATED HERE
========================================================================== */

window.addJourneySources = function () {
    const map = MAP();

    function ensureSrc(id) {
        if (!map.getSource(id)) {
            map.addSource(id, {
                type: "geojson",
    ...
    ensureSrc("journey-flight");
    ensureSrc("journey-drive");
    ensureSrc("journey-current");
    ...
    console.log("✓ Journey sources ready");
};
...
/* ==========================================================================
   GLOBAL SPIN ENGINE — FIXED FOR MODULAR SYSTEM
========================================================================== */
...
window.spinGlobe = function () {
    ...
    if (!window.spinning || window.userInterrupted) return;
...
window.focusJourneyOrbit = function (id) {
    ...
    const lowZoom = (id === "la" || id === "toronto") ? 6.25 : 12.5;
...
window.buildCompleteUntil = function (id) {
    ...
    for (let i = 0; i < idx; i++) {
        ...
        const isF = (a === "sydney" && b === "la") ||
                    (a === "la" && b === "toronto");

        if (isF) join(out.flight, buildGreatCircle(a, b));
        else     join(out.drive,  roadLeg(a, b));
    }
...
/* ==========================================================================
   JOURNEY ANIMATION — WITH CINEMATICS
========================================================================== */
...
window.animateLeg = function (a, b) {
    ...
    function anim(t) {
        ...
        const count = Math.max(2, Math.floor(totalPts * p));
        ...
        if (p < 1) {
            ...
            const done = buildCompleteUntil(b);

            map.getSource("journey-flight").setData({
                type: "Feature",
                geometry: { type: "LineString", coordinates: done.flight }
            });

            map.getSource("journey-drive").setData({
                type: "Feature",
                geometry: { type: "LineString", coordinates: done.drive }
            });
    ...
    /* ---- CINEMATIC SYD→LA EXACTLY PRESERVED ---- */
    if (a === "sydney" && b === "la") {
        const Syd = getWP(a);
        const LA  = getWP(b);
    ...
    /* ---- NORMAL (NON-CINEMATIC) LEG BEHAVIOR ---- */
    const wp = getWP(b);
...
/* ==========================================================================
   RESET JOURNEY — RESTORES STATIC ROUTES
========================================================================== */
...
window.resetJourney = function () {
    ...
    /* --- RESTORE STATIC LAYERS CREATED IN map-style.js --- */
...
/* ==========================================================================
   START JOURNEY — CLEAN MODULAR VERSION
========================================================================== */
